...
public partial class JobSeeker_JobSeekerSkillDetail : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        if (btnSave.Text == "Save")
        {
            try
            {
                ...
                int i = dl.JobSeekerSkillDetail_InsertData(bl);
                if (i > 0)
                {
                    lblMsg.ForeColor = System.Drawing.Color.Green;
                    lblMsg.Text = "Successfully Saved";
                    txtDiscription.Text = txtSkill.Text = "";
                    bind();
                    checkProfile();
                    if (Session["Percenteage"] != null)
                    {
            ...
            try {
                bl.Skill_ID = Convert.ToInt32(Session["Skill_ID"].ToString());
                bl.SkillName = txtSkill.Text;
                bl.Discription = txtDiscription.Text;
                int i = dl.JobSeekerSkillDetail_UpdateData(bl);
                if (i > 0)
                {
                    lblMsg.ForeColor = System.Drawing.Color.Green;
    ...
    public void bind()
    {
        ...
        {
            bl.JobSeeker_ID = Convert.ToInt32(Session["JobSeeker_ID"].ToString());
            DataTable dt = dl.JobSeekerSkillDetailBy_JobSeekerId_Display(bl).Tables[0];
            GVSkillInfo.DataSource = dt;
    ...
    protected void btnEdit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            txtSkill.Text = GVSkillInfo.DataKeys[gr.RowIndex].Values["SkillName"].ToString();
            txtDiscription.Text = GVSkillInfo.DataKeys[gr.RowIndex].Values["Discription"].ToString();
            Session["Skill_ID"] = GVSkillInfo.DataKeys[gr.RowIndex].Values["Skill_ID"];
            btnSave.Text = "Update";
        }
        catch
        {
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Button btnE = (Button)sender;
            GridViewRow gr = (GridViewRow)btnE.NamingContainer;
            bl.Skill_ID = Convert.ToInt32(GVSkillInfo.DataKeys[gr.RowIndex].Values["Skill_ID"].ToString());
            int i = dl.JobSeekerSkillDetail_DeleteData(bl);
            if (i > 0)
            {
                bind();
                lblMsg.ForeColor = System.Drawing.Color.Green;
